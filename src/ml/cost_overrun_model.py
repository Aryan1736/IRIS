"""IRIS PR-07 Cost Overrun Model Definitions, Feature Policy, Preprocessing, and Estimators.
...
baseline estimators, and model explainability for the 3-month cost-overrun target
...
def extract_features_at_t(
    ...
    return {
        ...
        "past_progress_stagnant_3m": int(prog_delta_3 == 0.0) if prog_delta_3 is not None else None,
...
def build_cost_overrun_modeling_dataset(
    ...
    for p_code, group in projects:
        ...
        for i, current in enumerate(rows):
            ...
            if decision.eligible:
                ...
                features["continuous_segment"] = segment_for_month(current["report_month"]) or "NONE"
                features["target_effective_cost_esc_3m"] = decision.label
                features["cost_revision_type"] = decision.cost_revision_type
...
class CostOverrunFoldPreprocessor:
    ...
    def fit(self, rows: Sequence[dict[str, Any]]) -> "CostOverrunFoldPreprocessor":
        if not rows:
...
def fit_logistic_candidate(
    ...
    candidate_name: str,
    feature_columns: Sequence[str] = FEATURE_NAMES,
) -> tuple[np.ndarray, np.ndarray, float, np.ndarray, CostOverrunFoldPreprocessor, LogisticRegression]:
    ...
    preprocessor = CostOverrunFoldPreprocessor(feature_columns).fit(training_rows)
    x_train = preprocessor.transform(training_rows)
    x_score = preprocessor.transform(scoring_rows)
    y_train = np.asarray([int(r["target_effective_cost_esc_3m"]) for r in training_rows], dtype=int)
...
def lagged_cost_rule_predictions(
    scoring_rows: Sequence[dict[str, Any]],
    all_regime_rows: Sequence[dict[str, Any]],
) -> np.ndarray:
    """Deterministic historical rule: predict 1 if n_prior_cost_revisions increased at T vs T-1."""
    index = {
        (r["project_code"], r["report_month"], r["continuous_segment"]): r
        for r in all_regime_rows
    }
    preds = np.zeros(len(scoring_rows), dtype=float)
    for i, r in enumerate(scoring_rows):
        prior_key = (
            r["project_code"],
            add_months(r["report_month"], -1),
            r["continuous_segment"],
        )
        prior = index.get(prior_key)
        if prior is not None:
            curr_c = int(r.get("n_prior_cost_revisions") or 0)
            prior_c = int(prior.get("n_prior_cost_revisions") or 0)
            if curr_c > prior_c:
                preds[i] = 1.0
    return preds
